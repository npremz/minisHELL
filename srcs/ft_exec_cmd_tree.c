...
/*   Updated: 2023/12/15 20:24:53 by lethomas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_exec_cmd_tree_rec(t_btree *cmd_tree, t_cmd_type operator_out,
	int **fd_pipe_in, pid_t *pid_child_tab)
{
	t_bool		do_continue;
	...
	{
		if (ft_launch_exec(cmd_tree, operator_out, fd_pipe_in, pid_child_tab))
			return (EXIT_FAILURE);
		return (EXIT_SUCCESS);
	}
