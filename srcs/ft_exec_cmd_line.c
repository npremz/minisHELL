...
/*   Updated: 2024/03/14 18:43:32 by lethomas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			while (cmd->in[i] != NULL)
			{
				if (cmd->type_in[i] == redirection_here_doc)
				{
					if (ft_redirection_here_doc(&fd_heredoc,
							cmd->in[i] + (cmd->in[i][0] == '\''))
						|| ft_save_fd_heredoc(fd_heredoc, cmd->in + i))
						return (EXIT_FAILURE);
				}
				i++;
			}
...
int	ft_exec_cmd_line(char *command_line, t_list **env)
{
	...
	if (ft_token_parenthesis(token_list)
		|| ft_create_cmd_list(token_list, &cmd_list)
		|| ft_listen_heredoc(cmd_list)
		|| ft_create_cmd_tree(cmd_list, &cmd_tree)
		|| ft_exec_cmd_tree(cmd_tree, env))
		return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
// if (DEBUG_MODE)
